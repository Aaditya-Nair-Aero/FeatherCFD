"""Compare BFECC clamp modes: 0=std, 1=none, 2=SDF-aware, 3=local."""
import moderngl
import numpy as np
import time
...
for mode in [0, 1, 2, 3]:
    print(f"\n{'='*60}")
    print(f"CLAMP_MODE={mode}")
    print(f"{'='*60}")

    cfd = CFD_System(ctx)
    cfd.use_vcycle = True
    cfd.vcycle_interval = 1
    cfd.set_advection_clamp_mode(mode)

    grid = cfd.grid_size
    U_inf = 2.0
    cfd.inflow_vel = (U_inf, 0.0, 0.0)
    for prog in [cfd.prog_forces, cfd.prog_advection, cfd.prog_projection,
                 cfd.prog_advection_density]:
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % 25 == 0 or step <= 3 or step == n_steps:
            ...
            print(f"  step={step:4d}  Cd={Cd:.4f}  Cl={Cl:.4f}  "
                  ...
                  f"u_max={cfd._read_max_velocity():.3f}")
    elapsed = time.perf_counter() - t_start
    print(f"  Mode {mode}: {n_steps} steps in {elapsed:.1f}s, "
          f"final Cl={all_cl[-1]:.4f}, max Cl={max(all_cl):.4f}")
