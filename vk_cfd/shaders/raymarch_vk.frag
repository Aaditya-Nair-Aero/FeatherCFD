...
layout(binding = 1) uniform usampler3D u_obstacle;
layout(binding = 2) uniform sampler3D u_sdf;
...
layout(binding = 3) uniform VizParams {
    int viz_mode;
    float density_scale;
    float step_size;
    int num_steps;
...
vec3 compute_vorticity(vec3 uv) {
    float d = 1.0 / float(params.grid_size);
    ...
    vec3 dv_dx = (v_px - v_nx) * 0.5 * float(params.grid_size);
    vec3 dv_dy = (v_py - v_ny) * 0.5 * float(params.grid_size);
    vec3 dv_dz = (v_pz - v_nz) * 0.5 * float(params.grid_size);
    return vec3(
        dv_dy.z - dv_dz.y,
        dv_dz.x - dv_dx.z,
        dv_dx.y - dv_dy.x
...
void main() {
    ...
    float t_end   = t_hit.y;
    float step = (t_end - t_start) / float(params.num_steps);
    step = max(step, params.step_size);
    ...
    for (int i = 0; i < params.num_steps; i++) {
        ...
        if (params.viz_mode == 0) {
            ...
            float render_radius = float(params.grid_size) * 0.1;
        ...
        if (val > 0.001 && alpha_scale > 0.001) {
            float alpha = clamp(val * params.density_scale * step * alpha_scale, 0.0, 0.15);
