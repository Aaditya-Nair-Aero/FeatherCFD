"""NACA 0012: compare RBGS iterations vs V-cycle."""
import moderngl
import numpy as np
import time
...
def run_test(cfg):
    """Run a NACA test with given configuration dict."""
    name = cfg['name']
    cfd = CFD_System(ctx)
    cfd.use_vcycle = cfg.get('use_vcycle', False)
    cfd.vcycle_interval = cfg.get('vcycle_interval', 5)
    cfd.jacobi_iters = cfg.get('jacobi_iters', 40)
...
tests = [
    ...
    {'name': 'RBGS_200', 'use_vcycle': False, 'jacobi_iters': 200},
]
